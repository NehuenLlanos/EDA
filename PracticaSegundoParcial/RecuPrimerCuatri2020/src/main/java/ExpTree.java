...
public class ExpTree implements ExpTreeService {
    ...
    private boolean evalRec(Node current){
        if(current == null){
            throw new RuntimeException("Is an invalid boolean operator");
        }
        // Verifico si es un boolean
        if(Utils.isBoolean(current.data)){
            // Si lo es retorno su valor de verdad
            return Utils.parseBoolean(current.data);
        }
        // Sino soy un operador
        boolean left = evalRec(current.left);
        boolean right = evalRec(current.right);
        for(Operator op : Operator.values()){
            if(op.getOp().compareTo(current.data) == 0){
                return op.apply(left, right);
            }
        }
        // Si llego aca es porque no lo encontre en los operadores
        throw new RuntimeException("Is an invalid boolean operator");
    }
...
//                if(!Utils.isBooleanOperator(n.data)){
...
//                }
    ...
    public static void main(String[] args) {
        ExpTreeService myExp = new ExpTree();
        // ( ( 2 + 3.5 ) * -10 )
